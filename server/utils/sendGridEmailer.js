...
function sendEmailToSpeaker(
  ...
  return new Promise((resolve, reject) => {
    ...
    let emailContent;
    if (approved && !pending) {
      emailContent = `Congratulations! Your request to 
       speak at ${meetupTitle} on ${meetupDate} has been approved.`;
    }
    ...
    if (pending) {
      emailContent = `Thank you for signing up to speak 
        ${meetupTitle} on ${meetupDate}. You will be notified 
        as soon as a SDJS admin reviews your request.`;
      sendEmailToAdmin(adminEmail,
        meetupDate,
        meetupTitle,
        speakerEmail,
        speakerName
...
function sendEmailToAdmin(adminEmail,
  meetupDate,
  meetupTitle,
  speakerEmail,
  speakerName
  ...
  return new Promise((resolve, reject) => {
    if (speakerEmail == undefined) {
      return reject(new Error('Speaker Email is undefined'));
    }
    ...
    let emailContent = `You have a pending speaker request from 
      ${speakerName} for ${meetupTitle} on 
      ${meetupDate}. would you like to approve or deny them.`;
    const email = {
      to: adminEmail,
      from: adminEmail,
      subject: 'SDJS Meetup Speaker Request',
...
function sendConfirmCancelToAdmin(
  confirm,
  meetupDate,
  meetupTitle,
  speakerName
  ...
  return new Promise((resolve, reject) => {
    ...
    let emailContent,
      verb;
    if (confirm) {
      emailContent = `${speakerName} has confirmed their talk on 
      ${meetupDate} for ${meetupTitle}`;
      verb = 'confirmed.';
    }
    if (!confirm) {
      emailContent = `${speakerName} has canceled their talk on 
      ${meetupDate} for ${meetupTitle}`;
      verb = 'canceled.';
    }
